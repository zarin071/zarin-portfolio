...
function GuestbookCard() {
  ...
  return (
    <div className="relative max-w-lg" style={{ transform: "rotate(-0.4deg)" }}>
      {/* Paper shadow layers */}
      <div className="absolute inset-0 rounded-2xl bg-subtle dark:bg-darkSubtle" style={{ transform: "rotate(1.2deg) translate(4px, 4px)" }} />
      <div className="absolute inset-0 rounded-2xl bg-subtle dark:bg-darkSubtle opacity-60" style={{ transform: "rotate(0.6deg) translate(2px, 2px)" }} />

      {/* Main paper */}
      <div className="relative rounded-2xl overflow-hidden border border-subtle dark:border-darkSubtle" style={{ background: "#FAFAF8" }}>
        {/* Lined paper */}
        <div
          className="absolute inset-0 pointer-events-none opacity-40 dark:opacity-20"
          style={{
            backgroundImage: "repeating-linear-gradient(transparent, transparent 27px, #D4D0C8 27px, #D4D0C8 28px)",
        ...
        <div className="relative px-8 pt-8 pb-8 ml-[72px]">
          <p className="font-sans text-[10px] uppercase tracking-[0.15em] text-warmGray dark:text-darkWarmGray mb-4">
            Sign the guestbook
          </p>
          ...
          <AnimatePresence mode="wait">
            {status === "done" && submitted ? (
              <motion.div
                ...
                className="py-2"
              >
                <p className="font-serif italic text-xl text-ink dark:text-darkInk mb-3 leading-snug">
                ...
                <p className="text-xs text-warmGray dark:text-darkWarmGray mt-3 opacity-70">
              ...
              >
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-transparent border-b border-subtle dark:border-darkSubtle text-sm text-ink dark:text-darkInk placeholder:text-warmGray/50 focus:outline-none focus:border-ink dark:focus:border-darkInk py-1 transition-colors"
                    />
                  </div>
                  <div>
                    <label className="block text-[10px] uppercase tracking-widest text-warmGray dark:text-darkWarmGray mb-1.5">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full bg-transparent border-b border-subtle dark:border-darkSubtle text-sm text-ink dark:text-darkInk placeholder:text-warmGray/50 focus:outline-none focus:border-ink dark:focus:border-darkInk py-1 resize-none transition-colors leading-7"
                  />
                </div>
        ...
        <div className="absolute top-0 bottom-0 left-0 w-[72px] flex flex-col items-center justify-start pt-6 gap-6">
          {Array.from({ length: 8 }).map((_, i) => (
            <div key={i} className="w-4 h-4 rounded-full border-2 border-subtle dark:border-darkSubtle bg-white dark:bg-darkBg" />
          ))}
        </div>
      </div>
    </div>
